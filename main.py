...
click_sound = pygame.mixer.Sound(path.join(snd_dir, 'click2.wav'))
fly_sound = pygame.mixer.Sound(path.join(snd_dir, 'trap2.wav'))
mini_sound = pygame.mixer.Sound(path.join(snd_dir, 'minigame.mp3'))
pop_sound = pygame.mixer.Sound(path.join(snd_dir, 'pop.mp3'))
pop_sound.set_volume(0.7)
trap_sound = pygame.mixer.Sound(path.join(snd_dir, 'trap.mp3'))
trap_sound.set_volume(2)

bubble_sound = []
for snd in ['bubbles1.mp3','bubbles2.mp3','bubbles3.mp3']:
    bubble_sound.append(pygame.mixer.Sound(path.join(snd_dir, snd)))
pygame.mixer.music.load(path.join(snd_dir, 'Wallcome.mp3'))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    random.choice(bubble_sound).play()
                    bubble = Bubble()
    ...
    while waiting:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if clik(WIDTH-150, HEIGHT-80, mouse_x, mouse_y, 60):
                    click_sound.play()
                    waiting = False
            elif event.type == pygame.KEYDOWN:
